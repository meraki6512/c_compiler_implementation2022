...
extern char *yytext;
int line_no = 1, syntax_err = 0, current_level = 0;
...
void initialize();
BOOLEAN checkParameterVoidType(A_TYPE* t); //add here
...
A_ID *makeIdentifier(char* s){
	...
	id->value = 0;
	id->prev = current_id;
	current_id = id;					//copy new id to current_id
	return (id);
}
...
A_TYPE *makeType(T_KIND k){
	...
	t->kind = k;					//NULL, ENUM, ARRAY, STRUCT, UNION, FUNC, POINTER, VOID
	t->size = 0;
	t->local_var_size = 0;
	t->element_type = NIL;
	t->field = NIL;
...
void checkForwardReference(){
	...
	id = current_id;
	while (id){
		if (id->level < current_level) break;					//at the same level
		t = id->type;
		if (id->kind == ID_NULL) 
			syntax_error(31, id->name);
		else if ((id->kind == ID_STRUCT) && (t->field == NIL))	//check if NIL
...
void setDefaultSpecifier(A_SPECIFIER* p){
	A_TYPE* t; // ???
	if (p->type == NIL) p->type = int_type; 		//default type: int
	if (p->stor == S_NULL) p->stor = S_AUTO;		//default storage: auto
}

//merge and update specifier
...
//link two declarator list id1 and id2
...
A_ID *setFunctionDeclaratorSpecifier(A_ID* id, A_SPECIFIER* p){
	...
	//check redeclaration
	a = searchIdentifierAtCurrentLevel(id->name, id->prev);				//NIL: redeclaration
	if (a)
	...
	a = id->type->field;
	while (a){
		if (strlen(a->name)){							//have name
			current_id = a;
			if (a->type==NIL)							//only have name
				syntax_error(23, "");
		}
		a = a->link;
	}
...
A_ID *setDeclaratorListSpecifier(A_ID* id, A_SPECIFIER* p){
	...
	a = id;
	while (a){
		if (strlen(a->name) && searchIdentifierAtCurrentLevel(a->name, a->prev)) //check redeclaration
...
A_ID *setParameterDeclaratorSpecifier(A_ID* id, A_SPECIFIER* p){
	...
	if (p->stor || checkParameterVoidType(p->type))
		syntax_error(14, "");
	setDefaultSpecifier(p);
	id = setDeclaratorElementType(id, p->type);
	id->kind = ID_PARM;
...
BOOLEAN checkParameterVoidType(A_TYPE* t){	//add here
	...
	A_ID* a;
	if (t==void_type){			//if another types exist before void type...
		if (t->field)
			return (TRUE); //err
	}
	else{						//if void type exists previously... 
		a = t->field;
		while (a) {
			if (a->type == void_type)
				return (TRUE); //err
